...
app.controller('controller', ['$scope', '$http', '$log', 'uiGridConstants', 'validate', 'notify', 'helper',
    function ($scope, $http, $log, uiGridConstants, validate, notify, helper) {
        ...
        $scope.calFee_footer = function () {
            ...
            $scope.FeePrice = ($scope.SubTotalPrice + $scope.ExtrabedPrice + $scope.ServicePrice) * (100 - discountGuest) * $scope.taxFee / 10000
        }
        ...
        $scope.SearchAvailability = function (addBookingByRoomTypeId = -1) {
            ...
            $http({
                ...
                if (addBookingByRoomTypeId > 0) {
                    $scope.autoAddFirstBooking(addBookingByRoomTypeId)
                }
        ...
        $scope.StepGuest = function () {
            if ($scope.AutoCheckAccept_Accommodation())
                return;
            $scope.Step = 2;
            console.log($scope.bookingChoose)
            $scope.addNewGuest()
        ...
        $scope.ApplyGuest_ForBooking = function () {
            ...
            var v3 = validate.isNullOrEmptySingleShowError({ value: $scope.guestForBooking.Email, key: 'Email' })
            var v4 = validate.isNullOrEmptySingleShowError({ value: $scope.guestForBooking.Phone, key: 'Phone' })
            if (v1 || v2 || v3 || v4) {
                notify.error('Giá trị không hợp lệ')
                return
            }
        ...
        $scope.AutoCheckAccept_Accommodation = function () {
            if ($scope.bookingChoose == null || $scope.bookingChoose.length == 0) {
                alert("Bạn phải chọn một loại phòng")
                return true;
            }
            return false
        }
        $scope.AutoCheckAccept_Guest = function () {
            ...
            if (v1 || v2 || v4) {
                alert("Thông tin khách không hợp lệ")
                return true
            }
            return false
        }
        ...
        $scope.splitService = function () {
            $scope.generalService = $scope.serviceChoose.filter(x => x.RoomTypeIdIndex == "-1");
            $scope.generalPersonal = $scope.serviceChoose.filter(x => x.RoomTypeIdIndex != "-1");
        }

        $scope.calculatorExpectedBalance = function () {
            var total = helper.round(($scope.ExtrabedPrice + $scope.SubTotalPrice + $scope.ServicePrice) * (100 - $scope.data.Guest.Discount) * (100 + $scope.taxFee) / 10000 - $scope.data.Amount)
            $scope.ExpectedBalance = total
        }
        $scope.calculatorBalanceDue = function () {
            $scope.data.BalanceDue = helper.round(($scope.ExtrabedPrice + $scope.SubTotalPrice + $scope.ServicePrice) * (100 - $scope.data.Guest.Discount) * (100 + $scope.taxFee) / 10000)
        }
        ...
        $scope.ConfirmReservation = function () {
            ...
            if ($scope.ExpectedBalance < 0) {
                alert("Giá không hợp lệ!");
                return;
            }
            if ($scope.data.DepositPecent < 0 || $scope.data.DepositPecent > 100) {
        ...
        //#region initpage
        $scope.autoAddFirstBooking = function (roomtypeId) {
            var item = $scope.roomtypeAvailability.find(x => x.RoomTypeId == roomtypeId)
            $scope.addBooking(item, $scope.roomIdInit)
        }
        $scope.initPage = function () {
            var checkin = $scope.FormatDate(helper.findGetParameter('checkin'))
            var checkout = $scope.FormatDate(helper.findGetParameter('checkout'))
            var typeBooking = helper.findGetParameter('typeBooking')
            $scope.roomIdInit = helper.findGetParameter('roomId')
            var roomTypeId = helper.findGetParameter('roomTypeId')
            $scope.data.TypeReservation = typeBooking
            $scope.FromDate = checkin
            $scope.ToDate = checkout
            $("#FromDate_Step1").val(checkin)
            $("#ToDate_Step1").val(checkout)
            $scope.SearchAvailability(roomTypeId)
        }
        $scope.initPage()
        //#endregion
    }])